...
/* ━━ 案B：セクションがパルスアニメーションで「触れる」を伝える ━━ */
type PlateSection = "region" | "classNum" | "hira" | "number";

const PULSE_CSS = `
@keyframes plate-pulse {
  0%, 100% {
    box-shadow: inset 0 0 0 2px rgba(255, 230, 0, 0.18), 0 0 0 0 rgba(255, 230, 0, 0);
    background: rgba(255,255,255,0.04);
  }
  50% {
    box-shadow: inset 0 0 0 2px rgba(255, 230, 0, 1), 0 0 18px 3px rgba(255, 230, 0, 0.5);
    background: rgba(255, 230, 0, 0.11);
  }
}
.plate-pulse-0 { animation: plate-pulse 4s ease-in-out infinite 0.0s; }
.plate-pulse-1 { animation: plate-pulse 4s ease-in-out infinite 1.0s; }
.plate-pulse-2 { animation: plate-pulse 4s ease-in-out infinite 2.0s; }
.plate-pulse-3 { animation: plate-pulse 4s ease-in-out infinite 3.0s; }
`;
...
function InteractivePlate({ plate, onTap }: {
  ...
  const tapStyle = (hasVal: boolean, pulseClass: string): React.CSSProperties => ({
    cursor: "pointer",
    borderRadius: 10,
    border: "none",
    ...
    <>
      ...
      <div style={{
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 12, paddingTop: 8 }}>
          <div
            className="plate-pulse-0"
            onPointerDown={() => onTap("region")}
            style={{ ...tapStyle(!!plate.region, "plate-pulse-0"), fontSize: 40, padding: "3px 10px" }}
          >
            {plate.region || "地名"}
          </div>
          <div
            className="plate-pulse-1"
            onPointerDown={() => onTap("classNum")}
            style={{ ...tapStyle(!!plate.classNum, "plate-pulse-1"), fontSize: 40, letterSpacing: 4, padding: "3px 10px" }}
          >
            {plate.classNum || "・・・"}
          </div>
        </div>
        ...
        <div style={{ flex: 1, display: "flex", alignItems: "center", position: "relative" }}>
          <div
            className="plate-pulse-2"
            onPointerDown={() => onTap("hira")}
            style={{ ...tapStyle(!!plate.hira, "plate-pulse-2"), position: "absolute", left: 0, fontSize: 64, lineHeight: 1, padding: "2px 8px" }}
          >
            {plate.hira || "あ"}
          </div>
          {/* 番号：レイアウト用外枠（pulse非対象）＋パルス対象内枠 */}
          <div style={{ flex: 1, marginLeft: 80, display: "flex", alignItems: "center", justifyContent: "center", overflow: "hidden" }}>
            <div
              className="plate-pulse-3"
              onPointerDown={() => onTap("number")}
              style={{
                ...tapStyle(!!plate.number, "plate-pulse-3"),
                fontSize: 104,
                display: "flex", alignSelf: "center", alignItems: "center", justifyContent: "center",
...
export default function FinalConfirmBPage() {
        ...
        <div className="flex w-full" style={{ gap: 32 }}>
          ...
          <div className="flex flex-col flex-1" style={{ gap: 18 }}>
            ...
            <SectionCard iconType="truck" title="車両情報">
              <div style={{ display: "flex", flexDirection: "column", padding: "14px 36px 18px 48px", gap: 10 }}>
                {/* プレート＋最大積載 */}
                ...
                <div style={{ width: 500, display: "flex", justifyContent: "center" }}>
                  <div style={{
                    ...
                    background: "rgba(255,230,0,0.15)", borderRadius: 12,
                    padding: "8px 20px", border: "1px solid rgba(255,230,0,0.4)",
                  }}>
                    <span style={{ fontSize: 18 }}>💡</span>
                    <span style={{ fontSize: 17, fontWeight: 700, color: "#78350F", letterSpacing: "0.04em" }}>
                      光っている箇所をそれぞれタップして修正できます
                    </span>
                  </div>
                </div>
              </div>
